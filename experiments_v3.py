# -*- coding: utf-8 -*-
"""
v3.0 新机制实验脚本：为四大扩展机制生成真实实验数据与图表
（基于 ai_brain_entity.py v3.0 核心：STDP / reward / perceive_image / BrainSwarm）

实验5：STDP 因果方向性（A→B 与 B→A 序列训练，呈现 A 后的回响探针对照）
实验6：多巴胺奖励调制学习（奖励组 vs 无奖励组）
实验7：群体文化传递（6 代链式传递：温习维系文化 vs 不温习文化灭绝）
实验8：多模态 embedding 感知（embedding 相似度 vs 脉冲模式重合度）
"""
import json
import math
import random
